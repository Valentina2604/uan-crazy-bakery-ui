...
export function OrderDetailsModal({ isOpen, onClose, order, dictionary }: OrderDetailsModalProps) {
  ...
  const formatDate = (dateString: string) => {
    ...
    return date.toLocaleDateString(dictionary.navigation.home === 'Inicio' ? 'es-ES' : 'en-US', {
      day: 'numeric', month: 'long', year: 'numeric'
    });
  };

  const formatCurrency = (value: number) => {
    return new Intl.NumberFormat(dictionary.navigation.home === 'Inicio' ? 'es-CO' : 'en-US', {
      style: 'currency',
      currency: 'COP',
